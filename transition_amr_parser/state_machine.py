...
class DepParsingStateMachine():
    ...
    def __init__(self, tokens):
        ...
        #self.full_slot=False
        #self.slot_token=False
        self.previous_nts=[]
        self.first_item="SHIFT"
        self.items=[]
    ...
    def applyAction(self, action):
        ...
        if base_action == 'SHIFT':
            ...
            #if len(self.previous_nts)==0:
            self.first_item='SHIFT'
            self.items.append('T')
            
        elif base_action == 'IN' or base_action == 'SL':
            self.nts.append(self.stack[-1])
            self.previous_action='NT'
            self.open_brackets+=1

            self.previous_nts.append(base_action)
            self.items.append('NT')
            ...
            popped_nt=self.previous_nts.pop()
            if popped_nt=='IN':
                self.first_item='IN'
    ...
    def get_valid_actions(self):
        ...
        """
        print('NTs',self.previous_nts)
        print('first',self.first_item)
        print('previous',self.previous_action)
        print('buffer',self.buffer)
        print('stack',self.stack)
        print('unary',self.unary)
        if self.unary==3:exit(0)
        """
        ...
        if len(self.buffer) > 0 and self.open_brackets>0:
            #if len(self.previous_nts)>0 and self.previous_nts[-1] == 'SL' and self.first_item=='IN':
              
            #    none_actions.append('NONE')
            #else: #if self.previous_nts[-1] == 'IN':
            
            valid_actions.append('SHIFT')
            

        if len(self.buffer) > 0 and self.previous_action!='NT':
            valid_actions.append('SL')
            valid_actions.append('IN')
        ...
        if len(self.buffer)==0 and (self.unary<3 and self.previous_action!='NT'):
            ...
            if self.items[-1]=='I':
                ...
                if len(self.previous_nts) > 0 and self.previous_nts[-1] == 'IN':
                    valid_actions.append('SL')
                elif len(self.previous_nts) == 0:
                    valid_actions.append('IN')
                    valid_actions.append('SL')
            """
        ...
        if len(self.stack) >= 1 and self.unary<4 and self.open_brackets>0:
            ...
            for i in range(len(self.items)):
                if self.items[-(i+1)]=='NT':
                    break
                span.append(self.items[-(i+1)])
            """    
            ...
            """
            if self.previous_nts[-1] == 'IN' and 'I' not in span:
                valid_actions.append('RE')

            if self.previous_nts[-1] == 'SL' and 'S' not in span:
                valid_actions.append('RE')
            """
            

            #HACK
            #if len(valid_actions)==0 and self.unary==3:
            valid_actions.append('RE')

        if len(self.stack) == 1 and len(self.buffer)==0 and self.previous_action=='RE':# and self.items[-1] == 'I':

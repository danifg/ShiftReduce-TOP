...
for single_task  in $(python -c "print(' '.join('$TASK_TAG'.split('+')))");do
	...
	python INOact2top.py ${results_path}.actions $ORACLE_FOLDER/dev.en > ${results_path}.top

	python EM.py $ORACLE_FOLDER/gold_dev.clean.tsv ${results_path}.top | tail -n 1 > ${results_path}.las
